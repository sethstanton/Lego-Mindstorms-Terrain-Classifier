    ...
    with open(file_name, 'r') as file:
        for line in file:
            if line.strip() and '[' in line and ']' in line:
                ...
                # These lines can be changed to skip multiple or single values must match up the label to the number
                #or it gets confused
                #data.append([float(val) for idx, val in enumerate(line_data) if idx >= 4 and idx not in (12, 4,5)])
                data.append([float(val) for idx, val in enumerate(line_data) if idx >= 4 and idx != 5])
    #df = pd.DataFrame(data, columns=[col for col in sensor_columns if col not in ('DownSensor','Pitch', 'Roll' )])
    df = pd.DataFrame(data, columns=[col for col in sensor_columns if col != 'Roll'])
...
"""
Here we are performing our stratified cross validation across each fold before being the total is being averaged.
This is done to calculate the average feature importance so that I can identify the top 5 features.
This data is then plotted on a graph and the top 5 features are printed to the console.
"""
...
# This line controls how many features are used for training
